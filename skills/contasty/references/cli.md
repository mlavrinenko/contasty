...
`PATH` is repeatable: files, directories, globs, or a saved query. Each unfolds
to a set of source files; the deduped, sorted union is stripped. Directories
are walked `.gitignore`-aware. Globs are expanded internally — quote them
(`'src/**/*.rs'`) so the shell does not expand them first. A glob matching
nothing warns; a missing literal path errors. Default path is `.`. `@name`
runs a saved query — see [Saved queries](#saved-queries-name).
...
| `--config=<PATH>`| Override the project config path (default: `.contasty/config.toml`). |
| `-h, --help`    | Print help.                                                       |
...
Config precedence: `.contasty/config.toml` (layered under the XDG global
config) loads first; any CLI `--strip` overrides it for all languages.
...
bare strings (required) or `{ path, required }`. Inline/list patterns, and
patterns read from an external file, always root at `cwd` (the directory you
run `contasty` from), not the query file's own directory — this is what makes
a saved query reusable across projects. The external `{ path }` file itself,
and import targets, are located relative to the query file's directory and
may live outside `cwd` (e.g. `.contasty/rules/` or the XDG global dir) with no
sandbox error. Cyclic imports are skipped; last matching pattern wins.

### Saved queries (`@name`)

`contasty @name` resolves a saved query: `<project>/.contasty/queries/
<name>.cty.yaml` (then `.cty.yml`), else `<global>/queries/<name>.cty.yaml`
(then `.yml`); first hit wins, else an error listing every path searched. The
resolved file unfolds exactly like a query file passed by path.

## Configuration (`.contasty/config.toml`)

Project layer: `.contasty/config.toml` in the project root, or the path
`--config` gives. Layered under the XDG global config
(`$XDG_CONFIG_HOME/contasty/config.toml`, or
`$HOME/.config/contasty/config.toml`), project winning on a shared key. All
fields optional.
...
and extends or overrides per-language strip rules — a grammar or rule file
registered once in the global config applies in every project. CLI `--strip`
overrides the config strip set for all languages.
...
`.contasty/config.toml`, no rebuild required.
